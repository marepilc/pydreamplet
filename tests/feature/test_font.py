...
def test_get_system_font_path_found():
    """
    Test that a generic CSS family resolves to an installed system font.
    """
    font_path = get_system_font_path("sans-serif", 400)
    assert font_path is not None, "A sans-serif font should be found on the system."
...
def test_get_system_font_path_accepts_css_fallback_list():
    font_path = get_system_font_path(
        "'This Font Does Not Exist', sans-serif", 400
    )

    assert font_path is not None, "The generic fallback should resolve."
...
def test_measure_text_returns_positive_dimensions():
    ...
    width, height = measurer.measure_text(
        "Test", font_family="sans-serif", weight=400, font_size=16
    )
...
def test_measure_text_uses_constructor_font_defaults():
    measurer = TypographyMeasurer(
        font_family="'This Font Does Not Exist', sans-serif",
        weight=400,
    )
